			...
			xmlhttp.onreadystatechange = function(){
				if (xmlhttp.readyState==4 && xmlhttp.status==200){
					if (xmlhttp.responseText){
						...
						function add(n) {
							var one = intent[n], node = one.parentNode, box = document.createElement("div"), width, height, bgurl = "/add/images/"+arr[n][2], bg = "background:url("+bgurl+") center no-repeat;", bgsize = '100%';
							if (node.className.indexOf('ad')>=0) {
								node.style.margin = 0;
								node = node.parentNode;
								one = one.parentNode;
							}
							function out(){
								box.setAttribute("style", "max-width:"+width+";height:"+height+";"+bg+"margin:0 auto 1em;cursor:pointer");
								if(arr[n][1].length) box.setAttribute("onClick", "location.href='"+arr[n][1]+"'");
								box.innerHTML = arr[n][0]; 
								node.insertBefore(box,one.nextSibling);
							}
							if (arr[n][0].length){
								width = '100%';
								height = 'auto';
								...
								bg += "background-size:"+bgsize+";";
								var image = new Image()
								image.onload = function(){
									var ratio = image.width/image.height, w = document.body.clientWidth, x = n==0 ? 414 : 336;
									width = Math.min(w, x, image.width);
						...
						if(arr[0].length) add(0);
						if(intent.length>1 && arr[1].length) add(1);
					}
				}
			}
